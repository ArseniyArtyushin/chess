...
def bishop_move(board, si, sj, fi, fj):
    ...
    dj = fj - sj
    if board[fj][fi] != SPACE:
        return board[sj][si] * board[fj][fi] < 0 and check_empty(board, si, sj, fi, fj)
...
def king_move(board, si, sj, fi, fj):
    ...
    dj = fj - sj
    if board[fj][fi] != SPACE:
        return board[sj][si] * board[fj][fi] < 0 and check_empty(board, si, sj, fi, fj) and abs(di) <=1 and abs(dj) <= 1
